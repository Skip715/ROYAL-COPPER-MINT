...
// ─── PRICING CONFIG — edit these markup amounts any time ───
const MARKUP = {
  bar500g:  15.00,  // £ added on top of copper melt value
  bar100g:   4.00,
  coin1oz:   5.00,
};
// Weights in kg
const WEIGHT = {
  bar500g:  0.500,
  bar100g:  0.100,
  coin1oz:  0.02835, // 1 troy oz in kg
};
// ───────────────────────────────────────────────────────────
...
function calcPrice(spotPerKg: number, weightKg: number, markup: number) {
  return spotPerKg * weightKg + markup;
}

async function fetchCopperSpot(): Promise<number | null> {
  try {
    // metals.live — free, no API key, returns USD/oz for metals
    // Copper LME price in USD per tonne from public source
    const res = await fetch("https://metals.live/api/v1/spot/copper", {
      next: { revalidate: 60 },
    });
    if (!res.ok) throw new Error("fetch failed");
    const data = await res.json();
    // metals.live returns price in USD per troy oz
    // Convert: USD/troy oz → USD/kg (× 32.1507) → GBP/kg (÷ 1.27 approx)
    const usdPerOz = data?.price ?? data?.[0]?.price ?? null;
    if (!usdPerOz) return null;
    const usdPerKg = usdPerOz * 32.1507;
    const gbpPerKg = usdPerKg / 1.27;
    return gbpPerKg;
  } catch {
    return null;
  }
...
async function fetchCopperSpotFallback(): Promise<number | null> {
  try {
    // Fallback: Frankfurter for USD→GBP rate + fixed copper reference
    const fxRes = await fetch("https://api.frankfurter.app/latest?from=USD&to=GBP");
    const fxData = await fxRes.json();
    const rate: number = fxData?.rates?.GBP ?? 0.79;
    // Copper LME ~$9,200/tonne as baseline if live feed fails
    const usdPerTonne = 9200;
    const gbpPerKg = (usdPerTonne / 1000) * rate;
    return gbpPerKg;
  } catch {
    return 8.42; // hardcoded last-resort fallback
  }
}

export default function Home() {
  const [spotPrice, setSpotPrice] = useState<number | null>(null);
  const [priceChange] = useState<number>(0.34);
  const [lastUpdated, setLastUpdated] = useState<string>("");
  const [isLive, setIsLive] = useState(false);

  useEffect(() => {
    async function loadPrice() {
      let price = await fetchCopperSpot();
      if (!price) {
        price = await fetchCopperSpotFallback();
        setIsLive(false);
      } else {
        setIsLive(true);
      }
  ...
  const prices = spotPrice
    ? {
        bar500g: calcPrice(spotPrice, WEIGHT.bar500g, MARKUP.bar500g),
        bar100g: calcPrice(spotPrice, WEIGHT.bar100g, MARKUP.bar100g),
        coin1oz: calcPrice(spotPrice, WEIGHT.coin1oz, MARKUP.coin1oz),
      }
    : null;

  const products = [
    {
      name: "500g Copper Bar",
      purity: "999.9 Fine Copper",
      price: prices ? formatPrice(prices.bar500g) : "…",
      img: "/ROYAL COPPER MINT BULLION.png",
      tag: "Best Seller",
    },
    {
      name: "100g Copper Bar",
      purity: "999.9 Fine Copper",
      price: prices ? formatPrice(prices.bar100g) : "…",
      img: "/ROYAL COPPER MINT BULLION.png",
      tag: "Popular",
    },
    {
      name: "Commemorative Coin",
      purity: "Limited Edition · 1oz",
      price: prices ? formatPrice(prices.coin1oz) : "…",
      img: "/ROYAL COPPER MINT COMMERORATIVE COIN.png",
      tag: "Collector's",
    },
  ];
  ...
  return (
    <main style={{ margin: 0, padding: 0, fontFamily: "'Garamond', 'EB Garamond', 'Cormorant Garamond', Georgia, serif", background: "#fff", color: "#1a1a1a", overflowX: "hidden" }}>
      ...
      <div style={{ background: "#1a1a1a", padding: "14px 60px", display: "flex", gap: "40px", alignItems: "center" }}>
        ...
        <span style={{ fontSize: "15px", color: "#D4956A", fontFamily: "Arial, sans-serif", fontWeight: "500" }}>
          {spotPrice ? `£${spotPrice.toFixed(2)} / kg` : "Loading…"}
        </span>
        <span style={{ fontSize: "12px", color: "#7BC67E", fontFamily: "Arial, sans-serif" }}>▲ {priceChange}%</span>
        <span style={{ color: "rgba(255,255,255,0.15)" }}>|</span>
        <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", letterSpacing: "0.2em", fontFamily: "Arial, sans-serif" }}>
          {isLive ? "Live" : "Indicative"}
        </span>
        <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: isLive ? "#7BC67E" : "#D4956A", display: "inline-block", animation: "pulse 2s infinite" }} />
        {lastUpdated && (
      ...
      <section style={{ maxWidth: "1200px", margin: "0 auto", padding: "100px 60px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "32px" }}>
          <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: isLive ? "#7BC67E" : "#D4956A", display: "inline-block", animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: "11px", color: "#999", fontFamily: "Arial, sans-serif", letterSpacing: "0.1em" }}>
            {isLive ? "Prices update live with the copper spot rate" : "Indicative prices based on current copper rate"}
          </span>
        </div>
              ...
              <div style={{ padding: "28px 24px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    ...
                    {spotPrice && (
                      <span style={{ fontSize: "10px", color: "#aaa", fontFamily: "Arial, sans-serif", display: "block", marginTop: "2px" }}>
                        Based on £{spotPrice.toFixed(2)}/kg spot
                      </span>
                    )}
                  </div>
